...
export default class ForceGraphPlugin extends Plugin implements HoverParent {
    ...
    this.registerEvent(
      this.app.workspace.on("active-leaf-change", (leaf) => {
        if (leaf && leaf.view.getViewType() === "markdown") {
          ...
          if (file) {
            console.log(`File navigated to: ${file.path}`);
  ...
  async openFileInFirstTab(filePath: string) {
    ...
    if (leftMarkdownLeaves.length > 0) {
  ...
  private getRightmostLeaf(leaves: WorkspaceLeaf[]): WorkspaceLeaf {
    if (leaves.length === 1) {
      return leaves[0];
    }
  ...
  public resetGlobalGraph = async (baseFolder: string) => {
    ...
    console.info(`Reset graph with max idx ${maxIdx - 1}`);
