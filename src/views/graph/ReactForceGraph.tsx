...
const ReactForceGraph: React.FC<GraphComponentProps> = ({
  ...
  function assignIdx(nodes: Node[]) {
    ...
    nodes.forEach((n) => {
      n.idx = maxIdx++;
      console.info(`Node ${n.path} idx = ${n.idx}`);
    });
  ...
  useEffect(() => {
    ...
    const initGraph = async () => {
      ...
      console.info(`Init Graph data starting with ${graph.rootPath}`);
  ...
  const expandNodeOp = async (nodePath: string | undefined, nodeIndex: number | undefined) => {
    ...
    console.info(`Nodes after expand: ${JSON.stringify(simplifiedNodes)}`);
    ...
    console.log(`New graph data after expansion: ${graphData.nodes.length + newNodes.length} 
      nodes and ${graphData.links.length + newLinks.length} links`);
  };

  const signalSelectedNode = (node: Node) => {
    if (!node) {
      console.info("Signal undefined selected!!!");
      return;
    }
  ...
  const handlePanRightLeftByIndex = (clockwise: boolean, lookChildSiblings: boolean) => {
    ...
    const nodeAngle = calcNodeAngle(rel, node);
    console.info("nodeAngle", nodeAngle);
